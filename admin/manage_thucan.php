...
// 1. Tầng xử lý logic - Thêm Thức ăn (Create)
if (isset($_POST['add'])) {
    $ten = $_POST['ten'];
    ...
    if ($image_error === 0) {
        ...
        if (in_array($image_extension, $valid_extensions)) {
            ...
            if (move_uploaded_file($image_tmp_name, $image_upload_path)) {
                ...
                $stmt->bind_param("ssssssssis", $ten, $danh_cho_loai, $mo_ta, $gia, $xuat_su, $thanh_phan, $cong_dung, $phu_hop, $soluong, $image_new_name);
                $stmt->execute();
                $stmt->close();
                header("Location: manage_thucan.php?status=success&message=Đã nhập thêm lương thảo vào kho rứa mô!");
                exit();
            }
        }
    }
...
// 2. Tầng xử lý logic - Xóa (Delete)
if (isset($_GET['delete'])) {
    $id = (int) $_GET['delete'];
    $conn->query("DELETE FROM thucan WHERE id = $id");
    header("Location: manage_thucan.php?status=success&message=Đã xóa bỏ thông tin thức ăn rứa!");
    exit();
}
...
// Xây dựng câu truy vấn SQL động (Dynamic Query Building)
$where_clauses = ["1=1"];

if ($search) {
    $search_esc = mysqli_real_escape_string($conn, $search);
    $where_clauses[] = "(ten LIKE '%$search_esc%' OR thanh_phan LIKE '%$search_esc%')";
}
if ($filter_type != 'all') {
...
// 4. Phân trang (Pagination) chuẩn hóa
$limit = 6;
$page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
$offset = ($page - 1) * $limit;

$total_res = $conn->query("SELECT COUNT(*) AS total FROM thucan WHERE $where_sql");
$total_records = $total_res->fetch_assoc()['total'];
$total_pages = ceil($total_records / $limit);

$result = $conn->query("SELECT * FROM thucan WHERE $where_sql ORDER BY id DESC LIMIT $limit OFFSET $offset");

// Lấy danh sách loài và xuất xứ hiện có trong kho để làm dropdown lọc (Distinct Values)
$types_res = $conn->query("SELECT DISTINCT danh_cho_loai FROM thucan WHERE danh_cho_loai != ''");
$origins_res = $conn->query("SELECT DISTINCT xuat_su FROM thucan WHERE xuat_su != ''");
...
<div id="content">
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="section-title mb-0">Quản lý thức ăn thú cưng</h2>
            <button id="toggleFormBtn" class="btn btn-pet shadow-sm">
                <i class="fa-solid fa-plus-circle me-2"></i> Thêm thức ăn mới
            </button>
        </div>

        <!-- FORM THÊM THỨC ĂN (Ẩn/Hiện) -->
        <div id="formContainer" class="form-container">
            <h5 class="fw-bold mb-4 text-secondary">
                <i class="fa-solid fa-bowl-food me-2 text-success"></i> Nhập thông tin dinh dưỡng mới rứa mô
            </h5>
            <form method="POST" enctype="multipart/form-data" class="row g-3">
                <div class="col-md-4">
                    <label class="form-label fw-bold">Tên thức ăn</label>
                    <input type="text" name="ten" class="form-control rounded-3" placeholder="Ví dụ: Hạt Royal Canin" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold">Dành cho loại</label>
                    <input type="text" name="danh_cho_loai" class="form-control rounded-3" placeholder="Ví dụ: Chó, Mèo" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold">Giá (VND)</label>
                    <input type="number" step="0.01" name="gia" class="form-control rounded-3" placeholder="Giá bán" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Số lượng trong kho</label>
                    <input type="number" name="so_luong" class="form-control rounded-3" placeholder="Số lượng" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Xuất xứ</label>
                    <input type="text" name="xuat_su" class="form-control rounded-3" placeholder="Pháp, Mỹ, Việt Nam...">
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Thành phần</label>
                    <input type="text" name="thanh_phan" class="form-control rounded-3" placeholder="Thịt gà, cá hồi...">
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Độ tuổi</label>
                    <input type="text" name="phu_hop_voi_tuoi_thu_cung" class="form-control rounded-3" placeholder="Mọi lứa tuổi">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-bold">Mô tả tổng quát</label>
                    <textarea name="mo_ta" class="form-control rounded-3" rows="2" placeholder="Thông tin thêm..."></textarea>
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-bold">Công dụng & Dinh dưỡng</label>
                    <textarea name="cong_dung" class="form-control rounded-3" rows="2" placeholder="Tốt cho hệ tiêu hóa..."></textarea>
                </div>
                <div class="col-md-12">
                    <label class="form-label fw-bold">Hình ảnh bao bì</label>
        ...
        <div class="filter-wrapper">
            <form method="get" class="row g-3 align-items-end">
                <div class="col-md-3">
                    ...
                    <input type="text" name="search" class="form-control rounded-3 shadow-none" placeholder="Tên thức ăn, thành phần..." value="<?= htmlspecialchars($search) ?>">
                </div>
                <div class="col-md-2">
                    <label class="form-label small fw-bold text-muted">Loài thú cưng</label>
                ...
                <div class="col-md-2">
                    ...
                    <select name="origin" class="form-select rounded-3 shadow-none">
                        <option value="all">-- Tất cả nơi --</option>
                            ...
                            <?php if ($result->num_rows > 0): ?>
                                <?php while ($row = $result->fetch_assoc()):
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <img src="<?= htmlspecialchars($imagePath) ?>" width="60" height="60" alt="Pet Food" onerror="this.src='../assets/image/default.jpg';">
                                        </td>
                                        <td>
                                            <div class="fw-bold text-dark"><?= htmlspecialchars($row['ten']) ?></div>
                                            <div class="small text-muted"><?= htmlspecialchars($row['phu_hop_voi_tuoi_thu_cung']) ?></div>
                                        </td>
                                        <td><span class="badge bg-light text-success border border-success rounded-pill px-3"><?= htmlspecialchars($row['danh_cho_loai']) ?></span></td>
                                        <td class="price-text"><?= number_format($row['gia'], 0, ',', '.') ?>₫</td>
                                        <td>
                                            <span class="fw-bold <?= ($row['so_luong'] < 10) ? 'text-danger' : 'text-dark' ?>">
                                                <?= (int) $row['so_luong'] ?>
                                            </span>
                                        </td>
                                        <td class="small text-secondary" style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                            <?= htmlspecialchars($row['thanh_phan']) ?>
                                        </td>
                                        <td class="text-center">
                                            <div class="btn-group shadow-sm rounded-pill overflow-hidden bg-white border">
                                                <a href="edit_thucan.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-primary border-0 py-2 px-3" title="Chỉnh sửa">
                                                    <i class="fa-solid fa-pen-to-square"></i>
                                                </a>
                                                <a href="?delete=<?= $row['id'] ?>" onclick="return confirm('Xác nhận xóa loại thức ăn này khỏi kho rứa mô?')" 
                                ...
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="8" class="text-center py-5 text-secondary">Không tìm thấy thức ăn nào phù hợp với yêu cầu lọc rứa mô!</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    function toggleForm() {
        if (formContainer.style.display === "none" || formContainer.style.display === "") {
            ...
            toggleFormBtn.innerHTML = '<i class="fa-solid fa-plus-circle me-2"></i> Thêm thức ăn mới';
        }
    }
