...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($full_name) || empty($phone) || empty($email) || empty($pet_name) || empty($pet_type) || empty($appointment_date) || empty($appointment_time) || empty($service)) {
        ...
        $stmt = $conn->prepare("INSERT INTO lichhen (ten_khach_hang, so_dien_thoai_khach_hang, email_khach_hang, ten_thu_cung, loai_thu_cung, ten_dich_vu, ngay_hen, gio_hen, ghi_chu,trang_thai) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
        if ($stmt) {
            $stmt->bind_param("ssssssssss", $full_name, $phone, $email, $pet_name, $pet_type, $service, $appointment_date, $appointment_time, $note,$trangthai);
            if ($stmt->execute()) {
