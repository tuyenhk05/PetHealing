    ...
    if ($image_error === 0) {
        ...
        if (in_array($image_extension, $valid_extensions)) {
            if (move_uploaded_file($image_tmp_name, $image_upload_path)) {
                ...
                header("Location: manage_phukien.php?status=success&message=Cập nhật sản phẩm thành công!");
                exit();
            } else {
                header("Location: manage_phukien.php?status=error&message=Lỗi khi tải ảnh lên.");
                exit();
            }
        } else {
            header("Location: manage_phukien.php?status=error&message=Chỉ hỗ trợ ảnh JPG, JPEG, PNG, GIF.");
            exit();
        }
    } else {
        // Không có ảnh mới
...
<body>
     ...
     <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="chat_lieu" value="<?php echo $row['chat_lieu']; ?>" required>
